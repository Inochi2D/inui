...
bool incGLBackendInit() {
    ...
    GLint major;
    GLint minor;
    glGetIntegerv(GL_MAJOR_VERSION, cast(GLint*)&major);
    glGetIntegerv(GL_MINOR_VERSION, cast(GLint*)&minor);
    g_GlVersion = cast(GLuint)(major * 100 + minor * 10);
    
    // OpenGL can't be older than 3.1
    enforce(!(major < 3 && minor < 1), "The OpenGL Context Version is too low (got %s.%s, requires %s.%s or newer).".format(major, minor, 3, 1));
...
void incGLBackendRenderDrawData(ImDrawData* draw_data) {
    float uiScale = Application.thisApp.uiScale;
        ...
        for (int cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd.UserCallback != null) {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f) {
                    ...
                    glScissor(cast(int) clip_rect.x, cast(int)(fb_height - clip_rect.w), cast(int)(
                            clip_rect.z - clip_rect.x), cast(int)(clip_rect.w - clip_rect.y));
                    ...
                    if (g_GlVersion >= 320)
                        glDrawElementsBaseVertex(GL_TRIANGLES, cast(GLsizei) pcmd.ElemCount, (ImDrawIdx.sizeof) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, cast(
                                void*) cast(int*)(pcmd.IdxOffset * (ImDrawIdx.sizeof)), cast(GLint) pcmd
                                .VtxOffset);
                    else
                        glDrawElements(GL_TRIANGLES, cast(GLsizei) pcmd.ElemCount, (ImDrawIdx.sizeof) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, cast(
                                void*) cast(int*)(pcmd.IdxOffset * (ImDrawIdx.sizeof)));
                }
            }
        }
    }
...
bool incGLBackendCheckShader(GLuint handle, string desc) {
    ...
    if (cast(GLboolean) status == GL_FALSE) {
        ...
        MessageBox.show(MessageType.error, "Compilation Failure", "Failed to compile shader %s!\nError:\n%s".format(desc, cast(string)buf));
    }
    return cast(GLboolean) status == GL_TRUE;
}

// If you get an error please report on GitHub. You may try different GL context version or GLSL version.
bool incGLBackendCheckProgram(GLuint handle, string desc) {
    GLint status = 0, log_length = 0;
    ...
    if (cast(GLboolean) status == GL_FALSE) {
        ...
        MessageBox.show(MessageType.error, "Compilation Failure", "Failed to link shader %s with GLSL '%s'!\nError:\n%s".format(desc, g_GlslVersionString, cast(string)buf));
    }
    return cast(GLboolean) status == GL_TRUE;
}
...
bool incGLBackendCreateDeviceObjects() {
    ...
    int glsl_version = g_GlslVersionString[9..$].to!int;
    ...
    // Create shaders
    const(GLchar)*[2] vertex_shader_with_version = [
        g_GlslVersionString.ptr, vertex_shader
    ];
    ...
    const(GLchar)*[2] fragment_shader_with_version = [
        g_GlslVersionString.ptr, fragment_shader
    ];
